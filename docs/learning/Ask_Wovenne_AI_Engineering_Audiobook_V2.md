# AI Engineering from Zero to Production — The Ask Wovenne Audiobook, Version 2
...
*A beginner-first spoken course built from the verified Ask Wovenne engineering record.*

## Source and truth note

The Wovenne-specific facts in this audiobook come from the supplied Ask Wovenne audiobook and companion study guide, both dated 30 August 2026 and described there as checked against the live repository and production database. This Version 2 preserves their factual boundaries. In particular, it does **not** turn offline evaluation results into claims about real-model accuracy, and it does **not** describe unfinished work as completed.

The short primer below adds general software and LLM vocabulary so a beginner can understand the project chapters that follow. Those explanations are teaching background; they are not new claims about what Wovenne has implemented.

## How to listen

Do not try to memorise every number on the first listen. On the first pass, learn the story: what the model does, what the application controls, what can go wrong, and how the system is bounded. On a second pass, learn the vocabulary and concrete Wovenne examples. On a third pass, answer the recall questions without looking at the notes.

A useful rule for this entire book is: when you hear a technical term, ask three questions. What does it mean? Where does it appear in Ask Wovenne? What failure does it prevent or help us understand?

---

# Part Zero — Absolute Beginner Primer

## Primer One. What is a software system?

Before talking about artificial intelligence, build a picture of an ordinary web application.

A customer sees a **frontend**. That is the part of the application presented in the browser: pages, buttons, forms, product cards and chat boxes.

Behind that is a **backend**. The backend is server-side software that handles operations the browser should not be trusted to control. It can validate requests, check identity and permissions, read or write data, call external services and decide what result to return.

Then there is usually a **database**. A database stores structured information such as products, stock, users or orders.

An **API** is a defined way for one piece of software to request something from another. In the source audiobook, the restaurant analogy is used: the customer does not enter the kitchen. The customer gives an order through a defined interface and receives a result.

Keep this picture in your head:

Customer. Frontend. Backend. Database or external service. Backend. Frontend. Customer.

In AI engineering, the language model is another external capability in that picture. It is not automatically the backend, the database or the security system.

**Memory checkpoint:** Which component should decide sensitive permissions: the browser, the language model, or deterministic backend code?

---

## Primer Two. Requests, responses, endpoints and JSON

When two software components communicate, one commonly sends a **request** and receives a **response**.

An **endpoint** is a particular address or route for a particular capability. You can think of it as a named counter in a large service desk. One counter might handle products; another might handle chat.

HTTP methods describe the kind of interaction. A GET request commonly asks to retrieve something. A POST request commonly sends information for processing or creation. The exact meaning is defined by the application, but that mental model is enough for now.

**JSON** is a common structured text format. It represents information as named fields and values. For example, a request might conceptually contain a field called message and a value containing the customer's question.

A response also has a **status**. Successful requests are in the two-hundred range. Four-hundred-range responses generally mean the request cannot be accepted as sent. Five-hundred-range responses generally mean the server or one of its dependencies failed to complete the operation.

Why does this matter for AI? Because an LLM integration is still software integration. A customer's message arrives as data. The backend validates it. The backend calls the model provider. The provider returns data. The backend decides what reaches the customer.

**Memory checkpoint:** Does JSON itself send data, or is JSON simply a format used to represent data in a request or response?

---

## Primer Three. Reading the TypeScript patterns you will hear about

You do not need to become a senior TypeScript developer to understand the architecture, but several patterns should stop looking mysterious.

A **variable** gives a value a name. An **object** groups related fields, such as a product name, price and stock state. An **array** is an ordered collection of values. A **function** is a named unit of behaviour that can receive inputs and return an output.

When code says **return**, it gives a value back to the code that called the function. Returning a value does not automatically mean displaying it on a webpage.

You will also hear **async** and **await**. Some operations take time: a network request, a database query, or a model call. An asynchronous function can wait for such work without treating it like an instant local calculation. `await` means, in simple terms, wait for this asynchronous operation's result before continuing with the next dependent step.

You will hear **try** and **catch** around risky operations. The idea is simple: try the operation; if it fails in an expected way, catch the failure and handle it deliberately instead of letting the whole request collapse without explanation.

That is enough syntax for the architecture chapters. The important question is not “can I memorise TypeScript punctuation?” It is “can I trace where input came from, what is trusted, what can fail, and what gets returned?”

**Memory checkpoint:** If a function returns product data, does that automatically mean the customer can see it on screen?

---

## Primer Four. What an LLM API call contains

The source material says Wovenne is configured to use Claude through Anthropic's API. An API call to a language model usually contains several categories of information.

First, a **model choice**: which model should answer.

Second, **messages**. A system message contains standing instructions. A user message contains the current user-side content. The word “user” in this message role is conversational structure; it is not proof that the person has authenticated successfully.

Third, limits such as a maximum output size.

Fourth, optional tool definitions that tell the model which controlled capabilities it may request.

The application also needs a secret credential, often called an **API key**, to authenticate to the model provider. A secret like that belongs on the server side, usually supplied through an environment variable. It should not be placed in browser code where visitors could inspect it.

This distinction is essential: the API key proves that the application may call the provider. It does not prove that an end customer is allowed to see a particular order. Provider authentication and customer authorization are different problems.

**Memory checkpoint:** What does the message role “user” mean, and what does it *not* mean?

---

## Primer Five. Training knowledge, context, tokens and stateless calls

A model's **training knowledge** is what it learned before your application called it. **Context** is the information your application supplies for the current call.

That distinction explains why a model should not be trusted to know live stock. Stock changes. The application must provide current evidence through context or a controlled tool.

Models process text in **tokens**, which are chunks of text rather than a perfect one-token-per-word mapping. Input tokens are the material sent to the model. Output tokens are what it generates.

The source audiobook emphasises that the relevant context is resent on each call. That matters in a multi-round tool loop: later calls can contain the system prompt, catalogue information, tool definitions, conversation and earlier tool results. The bundle can grow each round.

This is why context design affects both cost and latency.

One more term: **temperature** is a setting commonly associated with how variable model output can be. Lower variability can make behaviour more repeatable, but it does not turn an unsupported claim into a fact. Grounding still has to come from evidence.

**Memory checkpoint:** If you reduce response randomness, have you solved hallucination?

---

## Primer Six. The Ask Wovenne mental map

Now combine the pieces before entering Chapter One.

A customer asks a question. The application receives it. Deterministic code decides whether the feature is enabled, whether the caller is within limits and which capabilities are available. The model receives instructions and context. It may answer directly or request one of the controlled tools. The application validates and executes an allowed tool. The result comes back to the model as evidence. The model produces customer-facing language. The application records operational information about what happened.

The source material describes five tools: `search_products`, `get_product_details`, `check_availability`, `search_brand_knowledge`, and the privileged `get_my_order` tool. It also describes the loop as bounded: four tool rounds and then one forced tool-free round, for a maximum of five model calls per customer message.

The most important sentence to carry into the rest of the audiobook is the one used throughout the source material:

**The AI requests an action. The application decides whether and how that action happens.**

Do not move on until you can explain that sentence in your own words.

---

# Part One — The Model, Tools and Bounded Autonomy
...
Without looking back, answer this in one or two sentences: **Why is an LLM-powered feature different from ordinary deterministic software?**

If you cannot answer it yet, replay this chapter before moving on.
...
Without looking back, answer this in one or two sentences: **Who actually executes a tool request: the model or the application?**
...
Without looking back, answer this in one or two sentences: **Why is Ask Wovenne described as a bounded tool-using LLM rather than a fully autonomous agent?**
...
Without looking back, answer this in one or two sentences: **What may the model control, and what must deterministic code control?**

If you cannot answer it yet, replay this chapter before moving on.

# Part Two — Production Safety, Cost and Observability
...
Without looking back, answer this in one or two sentences: **Why is “deployed” different from “enabled”?**
...
Without looking back, answer this in one or two sentences: **Why can later rounds cost more even when the customer asked only one question?**
...
Without looking back, answer this in one or two sentences: **Why is six cents a ceiling rather than the expected price of a message?**
...
Without looking back, answer this in one or two sentences: **Why does a per-request cap not control total daily spend?**
...
Without looking back, answer this in one or two sentences: **What is check-then-act, and why does an atomic database operation fix the race?**
...
Without looking back, answer this in one or two sentences: **Why should a trace be opened before a request can be refused?**

If you cannot answer it yet, replay this chapter before moving on.

# Part Three — Evaluation and Failure Engineering
...
Without looking back, answer this in one or two sentences: **What makes an eval different from an exact-output unit test?**
...
Without looking back, answer this in one or two sentences: **What does 38 of 38 passing prove, and what does it not prove?**
...
Without looking back, answer this in one or two sentences: **Which kinds of protections should fail closed, according to the project reasoning?**
...
Without looking back, answer this in one or two sentences: **Why is “spent is below cap” an insufficient admission check?**
...
Without looking back, answer this in one or two sentences: **Why are integer micro-dollars safer than floating point for budget boundaries?**
...
Without looking back, answer this in one or two sentences: **Name the three live-eval guards and explain why no default spend cap exists.**

If you cannot answer it yet, replay this chapter before moving on.

# Part Four — Grounding, Hallucination and Factual Safety
...
Without looking back, answer this in one or two sentences: **What is grounding, and why can a hallucination be commercially dangerous?**
...
Without looking back, answer this in one or two sentences: **What are the three states of knowledge and why is “not checked” separate from “known absent”?**
...
Without looking back, answer this in one or two sentences: **Why can one invented certification fail a run even when several other facts are correct?**
...
Without looking back, answer this in one or two sentences: **Why must an honest sentence such as “we cannot confirm” be free from a grounding penalty?**

If you cannot answer it yet, replay this chapter before moving on.

# Part Five — Identity, Authorization and Data Boundaries
...
Without looking back, answer this in one or two sentences: **What is the difference between authentication and authorization?**
...
Without looking back, answer this in one or two sentences: **Where does the trusted customer identity come from for the order tool?**
...
Without looking back, answer this in one or two sentences: **Why is the customer-email filter a security control rather than a query convenience?**
...
Without looking back, answer this in one or two sentences: **Why was reflecting the caller's own order-reference input not automatically a data leak?**
...
Without looking back, answer this in one or two sentences: **What did the simulated PostgREST test prove, and what remained unproven?**
...
Without looking back, answer this in one or two sentences: **Why was matching a package name insufficient evidence that the real PostgREST server was available?**
...
Without looking back, answer this in one or two sentences: **Why keep both a fast boundary test and a slower high-fidelity integration test?**

If you cannot answer it yet, replay this chapter before moving on.

# Part Six — Adversarial Safety and Testing the Tests
...
Without looking back, answer this in one or two sentences: **Why is prompt wording alone an insufficient defence against prompt injection?**
...
Without looking back, answer this in one or two sentences: **What is the purpose of a small canary set before a larger paid evaluation?**
...
Without looking back, answer this in one or two sentences: **How does mutation testing prove that an evaluator can actually detect bad behaviour?**

If you cannot answer it yet, replay this chapter before moving on.

# Part Seven — Live Evaluation, Reliability and Architecture Choices
...
Without looking back, answer this in one or two sentences: **Why must model-quality metrics and operational metrics remain separate?**
...
Without looking back, answer this in one or two sentences: **Why should evaluation disable automatic provider retries even if production keeps them?**
...
Without looking back, answer this in one or two sentences: **Why is sixty cents for the first ten-case canary a maximum rather than a forecast?**
...
Without looking back, answer this in one or two sentences: **Why is one successful model response an anecdote rather than evidence of reliability?**
...
Without looking back, answer this in one or two sentences: **Why did the project decide not to add RAG yet?**
...
Without looking back, answer this in one or two sentences: **Why can missing product content be a larger AI-quality problem than the model choice?**
...
Without looking back, answer this in one or two sentences: **Why does having a second model-calling feature create a governance/inventory problem?**
...
Without looking back, answer this in one or two sentences: **Why must the feature flag be the last activation step?**

If you cannot answer it yet, replay this chapter before moving on.

# Part Eight — Competencies and Interview Defence
...
Without looking back, answer this in one or two sentences: **Which competency from this project can you explain with the strongest concrete evidence?**
...
Without looking back, answer this in one or two sentences: **What limitation should you state honestly when describing the evaluation work in an interview?**
...
# Final Revision Session — Answer from Recall

The companion study guide says recall, rather than rereading, is what moves this material into memory. Use the questions below after completing the audiobook. Answer aloud before reading the supplied answer key.
